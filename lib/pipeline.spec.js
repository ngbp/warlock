...
describe( 'Pipeline', function () {
  ...
  describe( '#_pipeline', function () {
    ...
    describe( 'when there is phase', function () {
      it( 'should filter out streams in a different named phase', function ( done ) {
        var startingValues = [ 1, 2, 3 ];
        var targetValues = startingValues.map( function ( v ) {
          return v * 2;
        });
        ...
        this.pipeline._options.source = _( startingValues );
        this.pipeline._options.phases = {
          prod: [ 'add' ]
        };

        this.pipeline._pipeline( 'dev' )
          .toArray( function ( values ) {
            values.should.eql( targetValues );
            done();
          });
      ...
      it( 'should leave in all streams not restricted to a named phase', function ( done ) {
        var startingValues = [ 1, 2, 3 ];
        var targetValues = startingValues.map( function ( v ) {
          return (v * 2) + 1;
        });
      ...
      it( 'should leave in streams restricted to the same named phase', function ( done ) {
        var startingValues = [ 1, 2, 3 ];
        var targetValues = startingValues.map( function ( v ) {
          return (v * 2) + 1;
        });
