...
Pipeline.prototype._pipeline = function () {
  var pipeline = this;
  var currentStream;
  ...
  function createErrorHandler ( errStream, prevStream ) {
    return function streamErrorHandler ( err ) {
      ...
      // don't stop the previous stream, it will keep throwing data down the pipeline this this
  ...
  this._streamIndex.forEach( function forEachStream ( name ) {
    addStream( pipeline._streams[ name ].generator() );
  });
