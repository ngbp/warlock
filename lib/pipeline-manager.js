          ...
          pipeline.on( 'end', function ( res ) {
            ...
            if ( join ) {
              ...
              // add them as a stream to the pipeline at the specified point.
                ...
                // By default, we concatenate this pipeline into the other pipeline, which means
                // these values will appear at the end of the stream. If the `reverse` property is
                // specified, we put these values from front of the other pipeline's values.
                if ( join.reverse ) {
                  concat = function ( otherStream ) {
                    return util.stream( res ).concat( otherStream );
                  };
                } else {
